...
vnuc = mf.get_nuc_att()
e_nuc_att_pyscf = np.einsum('ij,ji', vnuc, dm)
print()
print('PYSCF')
...
print('')
#
print('Vnuc parts test')
print('')
mydf = mf.with_df
print('df object: ', mydf)
vnuc_fft_at = pbctools.get_nuc_atomic_fftdf(mydf)
vnuc_fft = np.einsum('zab->ab', vnuc_fft_at)
print('all close pyscf vnuc and vnuc_fft?', np.allclose(vnuc, vnuc_fft, atol=1e-08) )
