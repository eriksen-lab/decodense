...
def sanity_check(mol: Union[gto.Mole, pbc_gto.Cell], \
                 ...
                 decomp: DecompCls, mo_coeff: Union[np.ndarray, Tuple[np.ndarray, np.ndarray]], \
        ...
        # mo coefficients
        assert isinstance(mo_coeff, np.ndarray) or isinstance(mo_coeff, tuple), \
        ...
        if isinstance(mo_coeff, np.ndarray):
            ...
            'invalid mo coefficients. must be a numpy array of dimension 2 or 3'
             ...
             assert len(mo_coeff) == 2 and isinstance(mo_coeff[0], np.ndarray) and isinstance(mo_coeff[1], np.ndarray), \
             'invalid mo coefficients. must be a tuple of two numpy arrays'
        # mo occupation
        assert mo_occ is None or isinstance(mo_occ, np.ndarray) or isinstance(mo_occ, tuple), \
            'invalid mo occupation. must be a numpy array or tuple of numpy arrays'
        if isinstance(mo_occ, np.ndarray):
            assert mo_occ.ndim == 1 or mo_occ.ndim == 2, \
            'invalid mo occupation. must be a numpy array of dimension 1 or 2'
        elif isinstance(mo_occ, tuple):
             assert len(mo_occ) == 2 and isinstance(mo_occ[0], np.ndarray) and isinstance(mo_occ[1], np.ndarray), \
