...
class _RSNucBuilder(_RSGDFBuilder):
    ...
    def _int_nuc_vloc(self, fakenuc:  pbc_gto.Cell, intor: str = 'int3c2e', \
        ...
        if is_zero(kpts):
            ...
            # here I don't sum over charges
            #vj_at1 = np.einsum('k...z,z->k...z', bufR, charge)
...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            if self.exclude_dd_block:
                print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
                print('int_vloc exclude_dd_blocke True')
...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            ...
            #atomic
            # here I don't sum over charges / looping over them
            for k in range(nkpts):
                if aosym == 's1':
                    for i in range(nchg):
...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        if self.exclude_dd_block:
            ...
            print('cell has naos, on these atoms', cell.nao, cell.bas_atom(np.arange(0, int(cell.nao/2))) )
            print('cell_d has naos, on these atoms', cell_d.nao, cell_d.bas_atom(np.arange(0, int(cell_d.nao/2)) ))
            for i in range(int(cell.nao/2)):
                print('cell', i, cell.bas_exp(i) ) 
            for i in range(int(cell_d.nao/2)):
                print('cell d', i, cell_d.bas_exp(i) ) 
            print(' ____________________')
            print('rs_cell has naos, on these atoms', self.rs_cell.nao, self.rs_cell.bas_atom(np.arange(0, int(self.rs_cell.nao/2))) )
            for i in range(int(self.rs_cell.nao/2)):
                print('rs_cell', i, self.rs_cell.bas_exp(i) ) 
            print(' ____________________')
            if cell_d.nao > 0 and fakenuc.natm > 0:
                ...
                if is_zero(kpts):
                    ...
                    print('are they close?', np.allclose(vj_dd, np.einsum('iabs->abs', vj_dd_at)) )
                    print('shape vj', np.shape(vj) )
                    print('are vj and vj_tot close?', np.allclose(vj_tot, np.einsum('aix->ax', vj)) )
                    vj1 = np.copy(vj)
                    vj_tot1 = np.copy(vj_tot)
# TODO continue here
...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        print('shape vj_tot an vj after merge_dd_at', np.shape(vj_tot), np.shape(vj))
        print('vj_tot and vj, allclose?', np.allclose(vj_tot, np.einsum('kix->kx', vj)) )
        #print('difference')
        #print(np.einsum('kix->kx', vj)-vj_tot)
        #print('is vj unchanged after merge?', np.allclose(vj, vj1))
        #print(vj-vj1)
        #print('is vj_tot unchanged after merge?', np.allclose(vj_tot, vj_tot1))
        #print(vj_tot-vj_tot1)
...
def _int_dd_block_at(dfbuilder, fakenuc, intor='int3c2e', comp=None):
    ...
    print('int dd atomic: vG', np.shape(vG), np.allclose(vG_tot, np.einsum('ix->x', vG) ) )
    ...
    if cell.dimension == 3:
        ...
        for z in range(len(charges)):
            ...
            print('z, vG_G0[z]', z, charges[z], vG_G0[z])
            vG[z,0] -= vG_G0[z]
    ...
    print('vR and vR_tot close?', np.allclose(np.einsum('ix->x', vR), vR_tot))
    ...
    if is_zero(kpts):
        ...
        print('0 ao_ks', np.shape(ao_ks[0]) )
        ...
        print('j3c', np.shape(j3c), np.allclose(j3c_tot, np.einsum('iabl->abl', j3c)))
...
def _merge_dd_at(rscell, aosym='s1'):
    ...
    def merge(j3c, j3c_dd, shls_slice=None):
        ...
        print('------------------------')
        print('my merge function')
        print('------------------------')
        print('j3c, j3c_dd', np.shape(j3c), np.shape(j3c_dd) )
        #print('ao_loc', ao_loc)
        #print('smooth_ao_idx', smooth_ao_idx)
        #print('nao rscell', rscell.nao)
        #print('nao cell', rscell.ref_cell.nao)
        #print('nprim cell', rscell.ref_cell.bas_nprim(smooth_bas_idx))
        #print('nprim rscell', rscell.bas_nprim(smooth_ao_idx))
        #print('off_ao_lists', offset_ao_lists)

        # get AO location/offset of every shell in the spherical basis, for each atom
        #picked_atom_list = offset_ao_lists[atm_idx]
        #start_id = picked_atom_list[0]
        #stop_id = picked_atom_list[1]
        #one_atom_ao_loc = ao_loc[np.argmax(ao_loc == start_id):np.argmax(ao_loc == stop_id)]
        #print('one_atom_ao_loc', atm_idx, one_atom_ao_loc)
        # get basis and then ao idx for each smooth/diffuse? basis function
        #smooth_bas_idx_at = rscell.bas_atom(smooth_bas_idx)
        #print('which atoms have smooth bas idx', smooth_bas_idx_at)
        #smooth_bas_idx_at = np.argwhere(smooth_bas_idx_at==atm_idx)
        # append the ao_loc with nao
        #one_atom_ao_loc = np.append(one_atom_ao_loc, np.size(smooth_bas_idx_at))
        #print('appended the one_atom_ao_loc', atm_idx, one_atom_ao_loc)
        #smooth_bas_idx_at = smooth_bas_idx_at.reshape(-1)
        #print('atomic smooth bas idx', smooth_bas_idx_at)
        #smooth_ao_idx_at = rscell.get_ao_indices(smooth_bas_idx_at, ao_loc)#one_atom_ao_loc)
        #print('smooth_ao_idx_at and smooth_ao_idx', smooth_ao_idx_at, smooth_ao_idx)
